...
class LogBase(object, metaclass=ABCMeta):
    ...
    def __init__(self, operator, headline, data, title, doctype,
        ...
        if self.time_range_flag:
            self.create_info = ' '.join([
                self.creator,
                _(u'operation.created at'),
                self.create_time.strftime('%F %T')
            ])
        else:
            self.create_info = self.creator + ' ' + \
                               _(u'operation.created at') + ' ' + \
                               self.create_time.strftime('%F %T') + '\n' + \
                               _(u'operation.data cycle') + ':' + \
                               self.start_time.strftime('%F %T') + ' - ' + \
...
class LogExporter(LogBase):
    ...
    def export_html(self):
        ...
        return BytesIO(html.encode()), self.export_filename + '.html'
    ...
    def export_pdf(self):
        ...
        stream.seek(0)
        return stream, self.export_filename + '.pdf'
    ...
    def export_xlsx(self):
        ...
        stream.seek(0)
        return stream, self.export_filename + '.xlsx'
